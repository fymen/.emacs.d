...
** Vertico
#+begin_src emacs-lisp :lexical no
(defun +minibuffer-file-highlight (cand)
  (when (string-suffix-p "/" cand)
    (add-face-text-property 0 (length cand) 'font-lock-function-name-face 'append cand))
  cand)

(defvar +minibuffer-category-highlight
  `((file . ,#'+minibuffer-file-highlight))
  "Custom highlight functions based on completion categories.")

(advice-add #'vertico--format-candidates :around #'vertico-format-candidates+)

(defun vertico-format-candidates+ (func metadata)
  (let* ((cat (completion-metadata-get metadata 'category))
         (hl-func (or (alist-get cat +minibuffer-category-highlight)
                      (alist-get t +minibuffer-category-highlight)
                      #'identity)))
    (cl-letf* (((symbol-function 'actual-vertico-format-candidate)
                (symbol-function #'vertico--format-candidate))
               ((symbol-function #'vertico--format-candidate)
                (lambda (cand prefix suffix index)
                  (actual-vertico-format-candidate
                   (funcall hl-func cand) prefix suffix index))))
...
(use-package elfeed
  ...
  (setq elfeed-feeds
        '(
          ...
          ("https://karthinks.com/software/index.xml", blog)
          ))
  )
#+end_src
* COMMENT EAF
